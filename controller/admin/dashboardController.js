    ...
    try {
        ...
        switch (timeFrame) {
            ...
            case 'custom':
                ...
                // Calculate appropriate date format and grouping based on date range
        ...
        if (timeFrame === 'daily') {
            ...
            if (daysDiff <= 1) {
                ...
                // Days in month
                chartData = Array.from({ length: daysInMonth }, (_, i) => {
                    const revenue = revenueData.find(d => d._id.month === (currentDate.getMonth() + 1) && d._id.day === (i + 1));
                    return {
                        time: `${format(new Date(currentDate.getFullYear(), currentDate.getMonth(), i + 1), 'MMM dd')}`,
                        revenue: revenue ? revenue.totalRevenue : 0
                    };
                });
            } else {
                ...
                chartData = Array.from({ length: 12 }, (_, i) => {
                    const revenue = revenueData.find(d => d._id.year === currentDate.getFullYear() && d._id.month === (i + 1));
                    return {
                        time: format(new Date(currentDate.getFullYear(), i), 'MMM yyyy'),
                        revenue: revenue ? revenue.totalRevenue : 0
                    };
                });
            }
